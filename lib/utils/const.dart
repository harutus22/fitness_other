...
const String calendarPlan = "calendar_plan";
...
const String planExerciseCalendarPass = "calendar_pass";
const String planExerciseTotal = "training_total";
const String planExerciseTime = "time";

const String privacyUrl = "https://sites.google.com/view/homeworkoutsforweightloss-priv/home-workouts-for-weight-loss-privacy-policy";
const String termsUrl = "https://sites.google.com/view/homeworkoutsforweightloss-term/home-workouts-for-weight-loss-terms-conditions";
...
Future<List<CalendarItem>?> getCalendar() async{
  SharedPreferences prefs = await SharedPreferences.getInstance();
  String? userPref = prefs.getString(calendarPlan);
  if(userPref == null)
    return null;
  List<dynamic> userMap = jsonDecode(userPref!) as List<dynamic>;
  List<CalendarItem> models = [];
  userMap.forEach((element) {
    models.add(CalendarItem.fromMap(element as Map<dynamic, dynamic>));
  });
  return models;
}

void setCalendar(List<CalendarItem> models) async{
  SharedPreferences prefs = await SharedPreferences.getInstance();
  // Map<String, dynamic> us = { for (var e in models) (e).name! : {(e)} };
  List<Map> us = [];
  for (var element in models) {us.add(element.toMap());};
  String res = jsonEncode(us);
  await prefs.setString(calendarPlan, res);
}
...
// void setSettings(AlarmSettings settings) async {
//   Alarm.set(alarmSettings: settings);
// }

const double onboardingTextSize = 28;